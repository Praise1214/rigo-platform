  ...
  const pushSiblings = (hoveredIdx: number) => {
    ...
    images.forEach((_, i) => {
      ...
      if (i === hoveredIdx) {
        ...
        const shiftedLeft = getPushedTransform(noRotation, -90)
        gsap.to(selector, {    
          transform: shiftedLeft,
          duration: 0.4,
          ease: 'back.out(1.4)',
          overwrite: 'auto',
        })
  ...
  return (
    ...
    >
      {images.map((src, idx) => (
        <div
          key={idx}
          className={`bounce-card bounce-card-${idx} absolute w-[300px] aspect-[3/4] border-[3px] border-white/80 rounded-2xl overflow-hidden`}
          style={{
            boxShadow: '0 8px 30px rgba(0, 0, 0, 0.15), 0 2px 8px rgba(0, 0, 0, 0.08)',
            transform: transformStyles[idx] || 'none',
            zIndex: idx, // ✅ base stack order
          }}
